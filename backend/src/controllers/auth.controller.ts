...
export async function registerHandler(
  ...
  try {
    ...
    try {
      ...
      return reply
        .code(201)
        .send({ accessToken: access, refreshToken: refreshRec.token, user });
    } catch (e: any) {
      if (
    ...
    return reply
      .code(400)
      .send({ error: "Invalid payload", details: err.errors ?? err.message });
  }
}
...
export async function refreshHandler(req: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    if (!v.ok) {
      return reply
        .code(401)
        .send({ error: "Invalid refresh token", reason: v.reason });
    }
    ...
    return reply.send({
      accessToken: access,
      refreshToken: rotated.newToken.token,
    });
  } catch (err: any) {
    return reply
      .code(400)
